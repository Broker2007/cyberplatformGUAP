...
const LabTemplatesPage = () => {
    ...
    const handleClearSearch = useCallback(() => {
        setLocalSearchValue('');
        dispatch(paginationActions.setSearchQuery({ query: '', scope: 'labs' }));
        dispatch(paginationActions.setPage({ page: 0, scope: 'labs' }));
    }, [dispatch]);

    useEffect(() => {
        if (debouncedSearchValue !== searchQuery) {
            dispatch(
                paginationActions.setSearchQuery({
    ...
    const handlePageChange = useCallback(
        (page: number) => {
            dispatch(paginationActions.setPage({ page, scope: 'labs' }));
        },
        [dispatch],
    );
