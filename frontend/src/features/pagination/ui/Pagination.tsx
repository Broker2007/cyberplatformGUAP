...
export const Pagination: React.FC<PaginationProps> = memo((props:PaginationProps) => {
    ...
    const handlePageClick = useCallback((page: number) => {
        setIsAnimating(true);
        dispatch(paginationActions.setPage({ page, scope }));
        onPageChange?.(page);

        setTimeout(() => setIsAnimating(false), 300);
    }, [dispatch, onPageChange, scope]);

    const handlePrevClick = useCallback(() => {
        if (hasPrevPage) {
            setIsAnimating(true);
            dispatch(paginationActions.prevPage({ scope }));
            onPageChange?.(currentPage - 1);
            setTimeout(() => setIsAnimating(false), 300);
        }
    }, [dispatch, onPageChange, hasPrevPage, currentPage, scope]);

    const handleNextClick = useCallback(() => {
        if (hasNextPage) {
            setIsAnimating(true);
            dispatch(paginationActions.nextPage({ scope }));
            onPageChange?.(currentPage + 1);
            setTimeout(() => setIsAnimating(false), 300);
        }
    }, [dispatch, onPageChange, hasNextPage, currentPage, scope]);
